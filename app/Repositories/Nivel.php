...
class Nivel
{
    ...
    public function findOne($id){
        $client = new \GuzzleHttp\Client();
        $full_path = env('API_URL').'levels/'.$id.'/?token='.$this->token;
    ...
    public function borrar($id) {
        $client = new \GuzzleHttp\Client();
        $response = $client->delete(env('API_URL').'levels/'.$id,array('form_params' => array('token' => $this->token)));
        $resp = json_decode($response->getBody()->getContents());
        if ($resp === NULL) {
            return 1;
        }else {
